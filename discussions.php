  ...
  <div id="main">
    <h2>
      <?php if ( is_home() or is_front_page() ) : ?>
        <?php _e( 'Recent Discussion', 'p2' ); ?> <?php if ( p2_get_page_number() > 1 ) printf( __( 'Page %s', 'p2' ), p2_get_page_number() ); ?>
      <?php else : ?>
        <?php printf( _x( 'Discussions from %s', 'Month name', 'p2' ), get_the_time( 'F, Y' ) ); ?>
      <?php endif; ?>
	...
	<?php while ( $query->have_posts() ) : $query->the_post(); ?>
          <?php if ( get_post_format() == FALSE ) : // This checks for 'standard' post formats ?>
            <?php p2_load_entry(); ?>
	  <?php endif; ?>
        <?php endwhile; ?>
